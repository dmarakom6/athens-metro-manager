...
void runSimulation()
{
    ...
    for (int i = 0; i < 8; ++i)
    {
        ...
        // we need fifo for this: (Station, Path to this Station)
        ...
        if (data.isMember("stations") && data["stations"].isArray())
        {
            for (const auto& station_json : data["stations"])
            {
                if (station_json.isMember("name") && station_json["name"].isString())
                {
                    ...
                    if (!position_found)
                    {
                        std::cerr << "Warning: Could not find a non-overlapping position for station '" << name <<
                            "' after " << MAX_ATTEMPTS << " attempts. Placing it anyway." << std::endl;
                    ...
                    if (station_json.isMember("connections") && station_json["connections"].isArray())
                    {
                        for (const auto& connection_name_json : station_json["connections"])
                        {
                            if (connection_name_json.isString())
                            {
                                ...
                                {
                                    std::cerr << "Warning: Connection to unknown station '" << connection_name <<
                                        "' for station '" << name << "'" << std::endl;
                                }
                            }
                        }
                    }
